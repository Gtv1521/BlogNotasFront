...
export class UserUseCase {
  ...
  constructor(@Inject(USER_CRUD_TOKEN) private user: IUser<User>) {}
  ...
  // busca data de usuario por email
  findByEmail(email: string): Observable<User[]> {
    return this.user.readXEmail(email).pipe(
      catchError((err) => {
        throw new Error('Error al obtener usuario', err);
      })
    );
  }
